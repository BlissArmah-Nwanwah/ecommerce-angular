...
import {FormControl, ReactiveFormsModule, Validators, ValidationErrors} from '@angular/forms';
...
describe('CustomInputFieldComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [CustomInputFieldComponent, ReactiveFormsModule, MatFormFieldModule, MatInputModule]
    }).compileComponents();
  ...
  const checkErrorMessage = (error: ValidationErrors, expectedMessage: string) => {
  ...
  it('should return invalid name error message if control has invalidName error', () => {
    ...
    checkErrorMessage({invalidName: true}, 'Did you entered your name correctly?');
  });

  it('should return email error message if control has email error', () => {
    initializeComponent('Email', '', new FormControl('', [Validators.required, Validators.email]));
  ...
  it('should return minlength error message if control has minlength error', () => {
    initializeComponent('Password', '', new FormControl('', [Validators.required, Validators.minLength(8)]));
    checkErrorMessage(
  ...
  it('should return custom domain error message if control has invalidDomain error with message', () => {
    initializeComponent('Website', '', new FormControl('', Validators.required));
    checkErrorMessage(

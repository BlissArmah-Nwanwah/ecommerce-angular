...
export class ProductService {
  ...
  private loadFromLocalStorage(): void {
    this.selectedProduct = this.localStorageService.getItem<ProductData>(
      this.selectedProductKey
    );
    this.cartProducts =
      this.localStorageService.getItem<CartProductData[]>(
        this.cartProductsKey
      ) || [];
    this.productCount =
      this.localStorageService.getItem<number>(this.productCountKey) || 0;
  }
  ...
  public createProduct(
    data: Pick<ProductData, 'title' | 'price' | 'description' | 'category'>
